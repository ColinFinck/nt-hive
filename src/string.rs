...
///
/// This allows a no_std caller to work with the string characters, even when it
/// cannot perform any allocations.
/// If the `alloc` feature is enabled, [`to_string_checked`] and [`to_string_lossy`]
...
impl<'a> NtHiveString<'a> {
    /// Attempts to convert the `NtHiveString` to an owned `String`.
