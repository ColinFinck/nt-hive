...
#[repr(packed)]
pub(crate) struct FastLeafElement {
...
pub struct FastLeafIter<'a, B: ByteSlice> {
    hive: &'a Hive<B>,
    elements_range: Range<usize>,
}

impl<'a, B> FastLeafIter<'a, B>
where
    B: ByteSlice,
{
    pub(crate) fn new(
        ...
        let elements_range =
            FastLeafElement::elements_range(count, count_field_offset, data_range)?;
...
impl<'a, B> Iterator for FastLeafIter<'a, B>
where
    B: ByteSlice,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        let key_node_offset =
            FastLeafElement::next_key_node_offset(&self.hive, &mut self.elements_range)?;
        ...
        self.elements_range.start += mem::size_of::<FastLeafElement>();
        Some(Ok(key_node))
    }

    fn size_hint(&self) -> (usize, Option<usize>) {
        let size = self.elements_range.len() / mem::size_of::<FastLeafElement>();
        (size, Some(size))
    }
}

impl<'a, B> ExactSizeIterator for FastLeafIter<'a, B> where B: ByteSlice {}
impl<'a, B> FusedIterator for FastLeafIter<'a, B> where B: ByteSlice {}

/// Iterator over mutable Fast Leaf Elements.
pub(crate) struct FastLeafIterMut<'a, B: ByteSliceMut> {
    hive: &'a mut Hive<B>,
    elements_range: Range<usize>,
}

impl<'a, B> FastLeafIterMut<'a, B>
where
    B: ByteSliceMut,
{
    pub(crate) fn new(
